                    ...
                    @if($planning->generated_plan && is_array($planning->generated_plan))
                        <div class="mt-6 grid lg:grid-cols-3 gap-6">
                            <div class="lg:col-span-2">
                                ...
                                <div class="space-y-3">
                                    @foreach(($planning->generated_plan['planning'] ?? []) as $slot)
                                        <div class="rounded-2xl border border-slate-100 p-4 bg-gradient-to-r from-white via-slate-50 to-emerald-50">
                                            <div class="flex items-center justify-between">
                                                <div>
                                                    <p class="font-semibold text-slate-900">{{ $slot['patient'] ?? 'Patient' }}</p>
                                                    <p class="text-sm text-slate-500">{{ $slot['time'] ?? 'Heure a confirmer' }}</p>
                                                </div>
                                            ...
                                            @if(!empty($slot['notes']))
                                                <p class="mt-2 text-sm text-slate-600">{{ $slot['notes'] }}</p>
                                            @endif
                                        </div>
                                    @endforeach
                                </div>
                                ...
                                <div class="space-y-3">
                                    @foreach(($planning->generated_plan['suggestions'] ?? []) as $suggestion)
                                        <div class="rounded-2xl border border-slate-100 p-4 bg-white">
                                            <p class="font-semibold text-slate-900">{{ $suggestion['patient'] ?? 'Patient' }}</p>
                                            <p class="text-sm text-slate-500">{{ $suggestion['suggested_time'] ?? 'Horaire a definir' }}</p>
                                            <p class="text-xs text-emerald-600 font-semibold mt-2">{{ $suggestion['priority'] ?? 'normal' }}</p>
